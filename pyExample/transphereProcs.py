...
def writeopac(opacity,localdust,nr):
    ...
    if localdust == 1:
        ...
        for ir in range(0,nr):
            for inu in range(0,nr): f.write(opacity['cabs'][inu]*redux)
            for inu in range(0,opacity['nf']): f.write(opacity['csca'][inu]*redux)
            f.write(' ')
        f.close
    else:
        ...
        f.write(str(opacity['nf'])+' 1\n')
        f.write(' '+'\n')
        for inu in range(0,opacity['nf']): f.write(str(opacity['cabs'][inu][0])+'\n')
        for inu in range(0,opacity['nf']): f.write(str(opacity['csca'][inu][0])+'\n')
        f.close
...
#        if nnf != nf: sys.exit('ERROR when reading opacity file in findKappa (nnf != nf)')
        ...
        for inu in range(1,nf-1):
            if (opacity['freq'][inu]-fvis)*(opacity['freq'][inu-1]-fvis) < 0.e0: ivis=inu
        eps    = (fvis-opacity['freq'][ivis-1]) / (opacity['freq'][ivis]-opacity['freq'][ivis-1])
...
#
    ...
    if model['tbg'] == 0.0 or model['isrf'] == 0: 
        ...
        if tbg > 0: bgspec=astroProcs.bplanck(model['freq'],model['tbg'])*model['isrf']
...
#
    ...
    for ir in range(0,len(model['r'])):
        f.write("%13.6E %13.6E %13.6E" % (model['r'][ir],model['rho'][ir],0.e0)+'\n') # ,format='(3(E13.6,1X))'
    f.close()
...
def readSpectrum(ext=0,file=0):
    ...
    readspectrum={'spectrum': spectrum, 'nfr': nrfr, 'freq': freq, 'fr_units': fr_units, 'obs': 0}
